...
DEFAULT BEHAVIOR:
    Shows the last 50 log lines from the past 5 minutes (context-friendly)
...
if [[ "$STREAM_MODE" == true ]]; then
    # Streaming mode
    CMD="log stream --predicate '$PREDICATE' --level $LOG_LEVEL"
    
    echo -e "${GREEN}Streaming VibeTunnel logs continuously...${NC}"
    echo -e "${YELLOW}Press Ctrl+C to stop${NC}\n"
else
    # Show mode
    CMD="log show --predicate '$PREDICATE'"
