...
export class SessionManagementTools {
  constructor(private api: VibeTunnelAPI) {}
  ...
  async executeListSessions(args: { includeExited?: boolean }) {
  ...
  async executeCreateSession(args: { name?: string; command: string[]; workingDir?: string }) {
  ...
  async executeSendInput(args: { session: string; text: string; addNewline?: boolean }) {
  ...
  async executeGetOutput(args: { session: string; lines?: number }) {
    const sessionResult = await this.resolveSession(args.session);
    if (!sessionResult.success) {
      throw new Error(sessionResult.error);
    }
  ...
  async executeGetSessionStatus(args: { session: string }) {
    const sessionResult = await this.resolveSession(args.session);
    if (!sessionResult.success) {
      throw new Error(sessionResult.error);
    }
  ...
  async executeKillSession(args: { session: string }) {
    const sessionResult = await this.resolveSession(args.session);
    if (!sessionResult.success) {
      throw new Error(sessionResult.error);
    }
  ...
  async executeRenameSession(args: { session: string; newName: string }) {
    const sessionResult = await this.resolveSession(args.session);
    if (!sessionResult.success) {
      throw new Error(sessionResult.error);
    }
  ...
  private async resolveSession(sessionIdentifier: string) {
