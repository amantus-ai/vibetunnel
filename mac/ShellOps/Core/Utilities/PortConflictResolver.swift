...
/// including identification details and helpers to determine if it's a ShellOps-related process.
struct ProcessDetails {
    ...
    /// Check if this is a ShellOps process
    var isShellOps: Bool {
        if let bundleId = bundleIdentifier {
            return bundleId.contains("shellops") || bundleId.contains("ShellOps")
        }
        if let path {
            return path.contains("ShellOps")
        }
        return self.name.contains("ShellOps")
    }

    /// Check if this is one of our managed servers
    var isManagedServer: Bool {
        // Direct shellops binary
        if self.name == "shellops" || self.name.contains("shellops") {
            return true
        }
        // Node server with ShellOps in path
        if self.name.contains("node"), path?.contains("ShellOps") ?? false {
            return true
        }
        // Bun executable (our shellops binary is a Bun executable)
        if self.name.contains("bun"), path?.contains("ShellOps") ?? false {
            return true
        }
        // Check if the path contains our bundle identifier
        if let path, path.contains("sh.shellops") {
            return true
        }
        return false
    }
...
/// for ShellOps-owned processes. Uses system tools like lsof to detect port usage
...
final class PortConflictResolver {
    ...
    /// Kill a process and optionally its parent ShellOps instance
    ...
    private func findRootProcess(for process: ProcessDetails) async -> ProcessDetails? {
        ...
        while let parentPid = current.parentPid, parentPid > 1, !visited.contains(parentPid) {
            ...
            if let parentInfo = await getProcessInfo(pid: parentPid) {
                // If parent is ShellOps, it's our root
                if parentInfo.isShellOps {
                    return parentInfo
                }
                current = parentInfo
            } else {
    ...
    private func determineAction(for process: ProcessDetails, rootProcess: ProcessDetails?) -> ConflictAction {
        ...
        // If root process is ShellOps, kill the whole app
        if let root = rootProcess, root.isShellOps {
            self.logger.info("Root process identified as ShellOps: \(root.name)")
            return .killOurInstance(pid: root.pid, processName: root.name)
        }

        // If the process itself is ShellOps
        if process.isShellOps {
            self.logger.info("Process identified as ShellOps: \(process.name)")
            return .killOurInstance(pid: process.pid, processName: process.name)
        }
