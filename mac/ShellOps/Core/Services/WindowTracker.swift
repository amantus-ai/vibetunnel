...
/// - Map ShellOps sessions to their terminal windows
...
///
/// ### Design Principles:
/// 1. **Only close what we open**: Windows are only closed if ShellOps opened them
...
///
/// ### User Experience:
/// - Consistent behavior: All ShellOps-spawned windows close on termination
...
final class WindowTracker {
    ...
    ///
    /// When ShellOps spawns a terminal session through AppleScript, we mark
    ...
    /// Registers a session that was opened by ShellOps.
    ...
    /// Closes the terminal window for a specific session if it was opened by ShellOps.
    ///
    /// This method implements a key feature where terminal windows are automatically closed
    /// when their associated sessions are terminated, but ONLY if ShellOps opened them.
    /// This prevents the common issue where killing a process leaves empty terminal windows.
    ///
    /// The method checks if:
    /// 1. The session was opened by ShellOps (exists in `sessionsOpenedByUs`)
    ...
    func closeWindowIfOpenedByUs(for sessionID: String) -> Bool {
        ...
        guard wasOpenedByUs else {
            self.logger.info("Session \(sessionID) was not opened by ShellOps, not closing window")
            return false
        }
