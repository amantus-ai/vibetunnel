...
/// Manager for ShellOps configuration stored in ~/.shellops/config.json
...
final class ConfigManager {
    ...
    private struct ShellOpsConfig: Codable {
        let version: Int
    ...
    private init() {
        ...
        self.configDir = homeDir.appendingPathComponent(".shellops")
    ...
    private func loadConfiguration() {
        ...
        if FileManager.default.fileExists(atPath: self.configPath.path) {
            do {
                ...
                let config = try JSONDecoder().decode(ShellOpsConfig.self, from: data)
