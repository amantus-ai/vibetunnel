...
/// Types of server events that can be received from the ShellOps server.
///
/// `ServerEventType` defines all possible event types that flow through the WebSocket v3 `EVENT`
/// frames between the ShellOps server and the macOS app. Each event type corresponds to
...
/// from the ShellOps server to the macOS app. Each event carries contextual information
