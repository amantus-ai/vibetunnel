...
final class PermissionChecker {
    ...
    func checkPermissions() -> Bool {
        if !self.checkPermissionsDirectly() {
            self.logger.warning("ShellOps needs accessibility permissions to focus terminal windows")

            let alert = NSAlert()
            alert.messageText = "Accessibility Permissions Required"
            alert.informativeText = """
            ShellOps needs accessibility permissions to focus terminal windows when you click on sessions.
