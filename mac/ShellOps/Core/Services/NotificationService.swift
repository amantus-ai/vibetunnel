...
/// Manages native macOS notifications for ShellOps events.
///
/// Connects to the ShellOps server to receive real-time events like session starts,
...
final class NotificationService: NSObject, @preconcurrency UNUserNotificationCenterDelegate {
    ...
    private let logger = Logger(subsystem: "sh.shellops.shellops", category: "NotificationService")
        ...
        case .notDetermined:
            ...
            do {
                ...
                if granted {
                    ...
                    content.title = "ShellOps Notifications"
    ...
    func sendNotification(for event: ServerEvent) async {
        ...
        case .testNotification:
            ...
            content.body = event.body ?? "ShellOps test notification"
    ...
    func sendTestNotification(title: String? = nil, message: String? = nil, sessionId: String? = nil) async {
        ...
        content.body = message ?? "This is a test notification from ShellOps"
        content.sound = self.getNotificationSound()
        content.categoryIdentifier = "TEST"
        content.interruptionLevel = .passive
    ...
    func openNotificationSettings() {
        ...
        if let url =
            URL(
                string: "x-apple.systempreferences:com.apple.Notifications-Settings.extension?id=sh.shellops.shellops")
        {
    ...
    private func handleTestNotification(_ event: ServerEvent) {
        let title = event.title ?? "ShellOps Test"
