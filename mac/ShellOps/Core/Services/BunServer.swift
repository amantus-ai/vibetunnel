...
/// Bun shellops server implementation.
///
/// Manages the Bun-based shellops server as a subprocess. This implementation
...
final class BunServer {
    ...
    func start() async throws {
        ...
        if devConfig.useDevServer, !devConfig.devServerPath.isEmpty {
            ...
            self.serverOutput.notice("🔧 ShellOps Development Mode - Hot reload enabled")
    ...
    private func startProductionServer() async throws {
        // Get the shellops binary path (the Bun executable)
        guard let binaryPath = Bundle.main.path(forResource: "shellops", ofType: nil) else {
            let error = BunServerError.binaryNotFound
            self.logger.error("shellops binary not found in bundle")
        ...
        if fileExists, !isDirectory.boolValue {
               ...
               let fileSize = attributes[.size] as? NSNumber
            {
                self.logger
                    .info(
                        "shellops binary size: \(fileSize.intValue) bytes, permissions: \(String(permissions.intValue, radix: 8))")
            }
        } else if !fileExists {
            self.logger.error("shellops binary NOT FOUND at: \(binaryPath)")
        }
        ...
        // Build the shellops command arguments as an array
        // Add Node.js V8 memory options first
        var shellopsArgs = ["--port", String(port), "--bind", bindAddress]
        ...
        case "none":
            shellopsArgs.append("--no-auth")
        case "ssh":
            shellopsArgs.append(contentsOf: ["--enable-ssh-keys", "--disallow-user-password"])
        case "both":
            shellopsArgs.append("--enable-ssh-keys")
        case "os", _:
        ...
        if authConfig.mode != "none" {
            ...
            shellopsArgs.append(contentsOf: ["--allow-local-bypass", "--local-auth-token", self.localAuthToken])
        ...
        if tailscaleServeEnabled {
            shellopsArgs.append("--enable-tailscale-serve")
            ...
            if tailscaleFunnelEnabled {
                shellopsArgs.append("--enable-tailscale-funnel")
        ...
        // Create wrapper to run shellops with parent death monitoring AND crash detection
        let parentPid = ProcessInfo.processInfo.processIdentifier

        // Properly escape arguments for shell
        let escapedArgs = shellopsArgs
            .map { arg in
        ...
        let shellopsCommand = """
        # Start shellops in background
        '\(binaryPath)' \(escapedArgs) &
        SHELLOPS_PID=$!

        # Monitor both parent process AND shellops process
        while kill -0 \(parentPid) 2>/dev/null && kill -0 $SHELLOPS_PID 2>/dev/null; do
            sleep 1
        done

        # Check why we exited the loop
        if ! kill -0 $SHELLOPS_PID 2>/dev/null; then
            # Shellops died - wait to get its exit code
            wait $SHELLOPS_PID
            EXIT_CODE=$?
            echo "ShellOps server process died with exit code: $EXIT_CODE" >&2
            exit $EXIT_CODE
        else
            # Parent died - kill shellops
            kill -TERM $SHELLOPS_PID 2>/dev/null
            wait $SHELLOPS_PID
            exit 0
        fi
        """
        process.arguments = ["-l", "-c", shellopsCommand]

        // Set up a termination handler for logging
        process.terminationHandler = { [weak self] process in
            self?.logger.info("shellops process terminated with status: \(process.terminationStatus)")
        }

        self.logger.info("Executing command: /bin/zsh -l -c \"\(shellopsCommand)\"")
    ...
    private func startDevServer(path: String) async throws {
        ...
        if ! kill -0 $PNPM_PID 2>/dev/null; then
            ...
            echo "🛑 ShellOps is shutting down, stopping development server..." >&2
...
enum BunServerError: LocalizedError, Equatable {
    ...
    var errorDescription: String? {
        switch self {
        case .binaryNotFound:
            "The shellops binary was not found in the app bundle"
        case .processFailedToStart:
