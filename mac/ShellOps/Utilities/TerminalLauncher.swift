...
///
/// Represents terminal emulators that ShellOps can launch
...
enum TerminalLauncherError: LocalizedError {
    ...
    var failureReason: String? {
        switch self {
        case .appleScriptPermissionDenied:
            return "ShellOps needs Automation permission to control terminal applications."
        case .accessibilityPermissionDenied:
            return "ShellOps needs Accessibility permission to send keystrokes to terminal applications."
...
final class TerminalLauncher {
        ...
        throws
    {
        ...
        // Check if we have a Bun executable (it would be bundled as shellops)
        let bunServerActive = Bundle.main.path(forResource: "shellops", ofType: nil) != nil &&
        ...
        if bunServerActive {
            ...
            if command.contains("TTY_SESSION_ID=") {
                ...
                // Format: TTY_SESSION_ID="..." shellops <actual_command>
                // We need to find where the actual command starts (after "shellops ")
                if let shellopsRange = command.range(of: "shellops ") {
                    let actualCommand = String(command[shellopsRange.upperBound...])
                    let bunCommand = self.buildForwarderCommand(
                        forwarderPath: forwarderPath,
            ...
            // For Go server, use shellops binary
            self.logger.info("Using Go server session creation via shellops binary")

            // Use provided shellops path or find bundled one
            let shellops = shellopsPath ?? self.findShellOpsBinary()

            // When called from Swift server, we need to construct the full command with shellops
            ...
            if command.contains("TTY_SESSION_ID=") {
                ...
                // Command is just the user command, need to add shellops
                fullCommand = "cd \"\(escapedDir)\" && TTY_SESSION_ID=\"\(sessionId)\" \(shellops) \(command) && exit"
            }
        }
    ...
    private func findShellOpsBinary() -> String {
        // Look for bundled shellops binary (shipped with the app)
        if let bundledShellops = Bundle.main.path(forResource: "shellops", ofType: nil) {
            self.logger.info("Using bundled shellops at: \(bundledShellops)")
            return bundledShellops
        }

        self.logger.error("No shellops binary found in app bundle, command will fail")
        return "echo 'ShellOps: shellops binary not found in app bundle'; false"
    }

    private func findForwarderBinary() -> String? {
        if let bundledForwarder = Bundle.main.path(forResource: "shellops-fwd", ofType: nil) {
            if FileManager.default.fileExists(atPath: bundledForwarder) {
                self.logger.info("Using bundled shellops-fwd at: \(bundledForwarder)")
    ...
    private func findBunExecutable() -> String {
        ...
        if let bundledPath = Bundle.main.path(forResource: "shellops", ofType: nil) {
        ...
        return "echo 'ShellOps: Bun executable not found in app bundle'; false"
    }
        ...
        -> String
    {
        ...
        let usesSubcommand = forwarderName != "shellops-fwd"
