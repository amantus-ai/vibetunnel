...
final class CLIInstaller {
        ...
        Task { @MainActor in
            ...
            for path in pathsToCheck where FileManager.default.fileExists(atPath: path) {
                ...
                if let content = try? String(contentsOfFile: path, encoding: .utf8) {
                    ...
                    // Allow for optional variables or arguments between $SHELLOPS_BIN and fwd
                    let hasValidExecCommand = content.range(
                        of: #"exec\s+["']?\$SHELLOPS_BIN["']?\s+fwd"#,
                        options: .regularExpression) != nil

                    if content.contains("ShellOps CLI wrapper"),
                       content.contains("$TRY_PATH/Contents/Resources/shellops"),
                       hasValidExecCommand
                    {
    ...
    func installCLITool() {
        ...
        confirmAlert
            .informativeText =
            "This will install the 'vt' command that runs ShellOps from your Applications folder. Administrator privileges are required."
    ...
    private func performInstallation() {
        ...
        # Clean up old shellops binary if it exists
        if [ -f "/usr/local/bin/shellops" ]; then
            rm -f "/usr/local/bin/shellops"
            echo "Removed old shellops binary"
        fi
        """
    ...
    private func showSuccess() {
        ...
        alert
            .informativeText =
            "The 'vt' command has been installed. You can now use 'vt' from the terminal to run ShellOps."
