...
/// Utility to detect and terminate other ShellOps instances
@MainActor
enum ProcessKiller {
    private static let logger = Logger(subsystem: BundleIdentifiers.loggerSubsystem, category: "ProcessKiller")

    /// Kill all other ShellOps instances except the current one
    static func killOtherInstances() {
        let currentPID = ProcessInfo.processInfo.processIdentifier
        self.logger.info("🔍 Current process PID: \(currentPID)")

        // Find all ShellOps processes
        let shellOpsProcesses = self.findShellOpsProcesses()

        // Kill other instances
        var killedCount = 0
        for process in shellOpsProcesses where process.pid != currentPID {
            logger.info("🎯 Found other ShellOps instance: PID \(process.pid) at \(process.path)")
        ...
        if killedCount > 0 {
            self.logger.info("🧹 Killed \(killedCount) other ShellOps instance(s)")

            // Give processes time to fully terminate
            Thread.sleep(forTimeInterval: 0.5)
        } else {
            self.logger.info("✨ No other ShellOps instances found")
        }
    }

    /// Find all running ShellOps processes
    private static func findShellOpsProcesses() -> [(pid: Int32, path: String)] {
        ...
        for process in allProcesses where process.path.contains("ShellOps.app/Contents/MacOS/ShellOps") {
            logger.debug("🎯 Found ShellOps process: PID \(process.pid) at \(process.path)")
            processes.append(process)
        }

        self.logger.info("📊 Found \(processes.count) ShellOps app processes")
        return processes
    }
