...
struct SessionDetailView: View {
                ...
                if let windowInfo {
                    ...
                    VStack(alignment: .leading, spacing: 12) {
                        if self.windowSearchAttempted {
                            ...
                            Text(
                                "Could not find a terminal window for this session. The window may have been closed or the session was started outside ShellOps.")
    ...
    private func updateWindowTitle() {
        ...
        if let pid = session.pid {
            self.windowTitle = "\(dir) — ShellOps (PID: \(pid))"
        } else {
            self.windowTitle = "\(dir) — ShellOps"
        }
    }
