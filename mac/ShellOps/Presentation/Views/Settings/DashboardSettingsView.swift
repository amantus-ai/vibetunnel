...
private let logger = Logger(subsystem: "sh.shellops.shellops", category: "DashboardSettingsView")
...
private struct ServerStatusSection: View {
    ...
    private func checkPortAvailability() async {
        ...
        if let conflict = await PortConflictResolver.shared.detectConflict(on: port) {
            // Only show warning for non-ShellOps processes
            // ShellOps instances will be auto-killed by ServerManager
