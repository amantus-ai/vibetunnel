                ...
                Section {
                    ...
                    HStack {
                        VStack(alignment: .leading, spacing: 4) {
                            ...
                            Text("Check for new versions of ShellOps.")
                                .font(.caption)
                                .foregroundStyle(.secondary)
                        }
...
private struct TerminalPreferenceSection: View {
    ...
    var body: some View {
        Section {
            VStack(alignment: .leading, spacing: 12) {
                ...
                HStack {
                    ...
                    Button("Test") {
                        Task {
                            do {
                                try self.terminalLauncher.launchCommand("echo 'ShellOps Terminal Test: Success!'")
                            } catch {
                                // Log the error
                                ...
                                if let terminalError = error as? TerminalLauncherError {
                                    ...
                                    case .appleScriptPermissionDenied:
                                        ...
                                        self.errorMessage =
                                            "ShellOps needs permission to control terminal applications.\n\nPlease grant Automation permission in System Settings > Privacy & Security > Automation."
                                    case .accessibilityPermissionDenied:
                                        self.errorTitle = "Accessibility Permission Required"
                                        self.errorMessage =
                                            "ShellOps needs Accessibility permission to send keystrokes to \(Terminal(rawValue: self.preferredTerminal)?.displayName ?? "terminal").\n\nPlease grant permission in System Settings > Privacy & Security > Accessibility."
            ...
            Text(
                "Configure which applications ShellOps uses for terminal sessions and Git repositories.")
                .font(.caption)
