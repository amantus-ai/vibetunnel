...
struct CLIInstallerTests {
    ...
    func scriptGeneration() throws {
        let sourcePath = "/Applications/ShellOps.app/Contents/Resources/vt"
    ...
    func scriptWithTitleModeArgsDetection() async throws {
        let script = """
        #!/bin/bash
        # ShellOps CLI wrapper
        for TRY_PATH in "/Applications/ShellOps.app" "$HOME/Applications/ShellOps.app"; do
            if [ -d "$TRY_PATH" ] && [ -f "$TRY_PATH/Contents/Resources/shellops" ]; then
                APP_PATH="$TRY_PATH"
                break
            fi
        done
        ...
        exec "$SHELLOPS_BIN" fwd $TITLE_MODE_ARGS "$@"
        """
