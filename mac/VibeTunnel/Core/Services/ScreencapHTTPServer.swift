...
public final class ScreencapHTTPServer {
    ...
    private func handleStartCapture(body: Data?, connection: NWConnection) async {
        ...
        guard let body = body,
              let request = try? decoder.decode(CaptureRequest.self, from: body) else {
        ...
        do {
            ...
            await sendJSON(connection: connection, data: ["status": "started", "type": request.type, "webrtc": String(useWebRTC)])
        } catch {
            await sendError(connection: connection, statusCode: 500, message: error.localizedDescription)
        }
    }
    ...
    private func handleCaptureWindow(body: Data?, connection: NWConnection) async {
        ...
        guard let body = body,
              let request = try? decoder.decode(WindowCaptureRequest.self, from: body) else {
            await sendError(connection: connection, statusCode: 400, message: "Invalid request body")
            return
        }
        
        // Find window by ID and start capture
        do {
            let windows = try await screencapService.getWindows()
            if let windowIndex = windows.firstIndex(where: { $0.cgWindowID == request.cgWindowID }) {
                let useWebRTC = request.webrtc ?? false
                try await screencapService.startCapture(type: "window", index: windowIndex, useWebRTC: useWebRTC)
                await sendJSON(connection: connection, data: ["status": "started", "cgWindowID": String(request.cgWindowID), "webrtc": String(useWebRTC)])
            } else {
                await sendError(connection: connection, statusCode: 404, message: "Window not found")
            }
        } catch {
            await sendError(connection: connection, statusCode: 500, message: error.localizedDescription)
        }
    ...
    private func handleClick(body: Data?, connection: NWConnection) async {
        ...
        guard let body = body,
              let request = try? decoder.decode(ClickRequest.self, from: body) else {
            await sendError(connection: connection, statusCode: 400, message: "Invalid request body")
            return
        }
    ...
    private func handleClickWindow(body: Data?, connection: NWConnection) async {
        ...
        guard let body = body,
              let request = try? decoder.decode(ClickWindowRequest.self, from: body) else {
            await sendError(connection: connection, statusCode: 400, message: "Invalid request body")
            return
        }
    ...
    private func handleKey(body: Data?, connection: NWConnection) async {
        ...
        guard let body = body,
              let request = try? decoder.decode(KeyRequest.self, from: body) else {
            await sendError(connection: connection, statusCode: 400, message: "Invalid request body")
            return
        }
    ...
    private func sendJSON<T: Encodable>(connection: NWConnection, data: T, statusCode: Int = 200) async {
        do {
