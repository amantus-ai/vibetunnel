...
final class WindowTracker {
        ...
        if (terminalApp == .terminal && tabReference != nil) ||
            ...
            Task {
                ...
                if let windowInfo = findWindow(
                    ...
                    logger.info("Successfully registered window \(windowInfo.windowID) for session \(sessionID) with explicit ID")
                }
            }
            return
        }
        ...
        Task {
            ...
            for (index, delay) in delays.enumerated() {
                ...
                if let windowInfo = findWindow(
                    ...
                    logger.info("Successfully registered window \(windowInfo.windowID) for session \(sessionID) after \(index + 1) attempts")
                    return
                }
            }
    ...
    private func findWindowForSession(_ sessionID: String, sessionInfo: ServerSessionInfo) -> WindowEnumerator.WindowInfo? {
        let allWindows = WindowEnumerator.getAllTerminalWindows()
        
        if let window = windowMatcher.findWindowForSession(sessionID, sessionInfo: sessionInfo, allWindows: allWindows) {
