...
final class UnixSocketConnection {
    ...
    nonisolated private func establishConnection() {
        Task { @MainActor in
    ...
    nonisolated private func waitForConnection() {
    ...
    private func handleSendError(_ error: Error, errorCode: Int32) {
        ...
        // Check for other connection errors
                ...
                errorCode == ENOTCONN { // 57 - Not connected
    ...
    private func scheduleReconnect() {
        guard shouldReconnect && !isReconnecting else {
            logger.debug("Skipping reconnect: shouldReconnect=\(self.shouldReconnect), isReconnecting=\(self.isReconnecting)")
            return
        }
        ...
        logger.info("🔄 Scheduling reconnection in \(String(format: "%.1f", self.reconnectDelay)) seconds (attempt #\(self.consecutiveFailures + 1))")
    ...
    private func sendKeepAlive() async {
        ...
        if timeSinceLastPong > keepAliveInterval * 2 {
            logger.warning("⚠️ No pong received for \(String(format: "%.0f", timeSinceLastPong))s, connection may be dead")
