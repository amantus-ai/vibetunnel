# Git Hooks in ShellOps

## Overview

ShellOps uses Git hooks exclusively for its **follow mode** feature. These hooks monitor repository changes and enable automatic branch synchronization across team members.

## Purpose

Git hooks in ShellOps serve a single, specific purpose:
...
**Important**: If you're not using follow mode, git hooks are not needed and serve no other purpose in ShellOps.

## How It Works

### Hook Installation

When follow mode is enabled, ShellOps installs two Git hooks:
...
2. **Hook Trigger**: Git executes the ShellOps hook
3. **Event Notification**: `vt git event` sends repository path to ShellOps server
...
1. ShellOps monitors checkouts to the followed branch
...
```bash
#!/bin/sh
# ShellOps Git hook - post-checkout
# This hook notifies ShellOps when Git events occur
...
```bash
git config shellops.followBranch <branch-name>
```
...
- **Automatic**: Managed by ShellOps when enabling/disabling follow mode
