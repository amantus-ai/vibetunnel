...
/// Service for discovering ShellOps servers available via Tailscale network
@Observable
@MainActor
final class TailscaleDiscoveryService {
    ...
    private func discoverServers() async {
        ...
        // Filter devices that could be ShellOps servers
        var newServers: [TailscaleServer] = []

        for device in tailscaleService.devices {
            logger
                .info(
                    "Checking device: \(device.name), OS: '\(device.os ?? "nil")', isOnline: \(device.isOnline), isShellOpsServer: \(device.isShellOpsServer), lastSeen: \(device.lastSeen ?? "nil")"
                )

            // Only check online devices that could be servers
            guard device.isOnline && device.isShellOpsServer else {
                logger
                    .info(
                        "Skipping device \(device.name): online=\(device.isOnline), server=\(device.isShellOpsServer), OS='\(device.os ?? "nil")'"
                    )
                continue
            }

            logger.info("Probing ShellOps on \(device.name) (\(device.ipv4Address ?? "no IP"))")

            // Check if this device has ShellOps running on port 4020
            if let server = await probeServer(hostname: device.name, ip: device.ipv4Address) {
                newServers.append(server)
                logger.info("Found ShellOps server: \(device.name)")
            } else {
                logger.info("No ShellOps response from \(device.name)")
            }
        }
        ...
        logger.info("Discovery complete. Found \(discoveredServers.count) ShellOps servers")
        isDiscovering = false
    }
        ...
        async -> TailscaleServer?
    {
        ...
        // Construct URL for health check - ShellOps uses /api/health endpoint
