...
/// Represents a discovered ShellOps server.
...
/// Service for discovering ShellOps servers on the local network using Bonjour/mDNS
@MainActor
@Observable
final class BonjourDiscoveryService: BonjourDiscoveryProtocol {
    ...
    func startDiscovery() {
        ...
        logger.info("Starting Bonjour discovery for _shellops._tcp services")

        // Clear existing servers
        self.discoveredServers.removeAll()

        // Create browser for ShellOps services
        let parameters = NWParameters()
        parameters.includePeerToPeer = true

        self.browser = NWBrowser(for: .bonjour(type: "_shellops._tcp", domain: nil), using: parameters)
...
/// Sheet view for discovering ShellOps servers on the local network.
            ...
            VStack {
                if self.discoveryService.isDiscovering, self.discoveryService.discoveredServers.isEmpty {
                    VStack(spacing: 20) {
                        ProgressView()
                            .scaleEffect(1.5)
                        Text("Searching for ShellOps servers...")
                    ...
                    VStack(spacing: 20) {
                        ...
                        Text("No servers found")
                            .font(.title2)
                        Text("Make sure ShellOps is running on your Mac\nand both devices are on the same network")
