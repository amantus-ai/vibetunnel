...
struct TailscaleSettingsContent: View {
                        ...
                        Button("Save") {
                            ...
                            Task {
                                ...
                                if tailscaleService.isRunning {
                                    if enableDiscovery {
                                        // Start discovery to find ShellOps servers
    ...
    var settingsSection: some View {
        VStack(alignment: .leading, spacing: Theme.Spacing.medium) {
            ...
            VStack(alignment: .leading, spacing: 4) {
                Text("When enabled, ShellOps will automatically use Tailscale for remote connections when available.")
                    .font(.caption)
            ...
            VStack(alignment: .leading, spacing: Theme.Spacing.medium) {
                ...
                VStack(spacing: Theme.Spacing.small) {
                    if discoveryService.isDiscovering {
                        ...
                        Text("No ShellOps servers found on Tailscale network")
                            .foregroundColor(.secondary)
                            .font(.caption)
                            .padding()
                    } else {
