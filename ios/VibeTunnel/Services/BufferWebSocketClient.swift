...
class BufferWebSocketClient: NSObject {
    ...
    private func sendV3Subscribe(sessionId: String) async throws {
        // Include all flags: stdout for live output, snapshots for buffer sync, events for exit/resize
        let flags: UInt32 = V3SubscribeFlags.stdout.rawValue | V3SubscribeFlags.snapshots.rawValue | V3SubscribeFlags.events.rawValue
