    ...
    func connectToProfile(_ profile: ServerProfile) async throws {
        ...
        do {
            ...
            if profile.isTailscaleEnabled, config.httpsAvailable, config.preferSSL, !fallbackAttempted {
                self.connectionLogger.warning("🔗 ⚠️ HTTPS connection failed, trying HTTP fallback")
                // connectionLogger.info("🔗 Error type: \(String(describing: type(of: error)))")
                ...
                if let healthProfile = await checkServerHealth(for: profile) {
                    ...
                    ServerProfile.save(healthProfile, to: self.userDefaults)

                    // Create new config from health check results
                    guard var newConfig = healthProfile.toServerConfig() else {
                        self.connectionLogger.error("🔗 ❌ Failed to create config from health check")
                        throw APIError.invalidURL
                    }
                    ...
                    self.connectionManager.saveConnection(newConfig)
                    ...
                    do {
                        ...
                        if authConfig.noAuth {
                            _ = try await APIClient.shared.getSessions()
                            self.connectionManager.isConnected = true
                            ServerProfile.updateLastConnected(for: healthProfile.id, in: self.userDefaults)
                            self.loadProfiles()
                    ...
                    self.connectionLogger.warning("🔗 ⚠️ Health check failed, trying blind HTTP fallback")
                    ...
                    do {
                        ...
                        if authConfig.noAuth {
                            _ = try await APIClient.shared.getSessions()
                            self.connectionManager.isConnected = true
                            ServerProfile.updateLastConnected(for: updatedProfile.id, in: self.userDefaults)
                            self.loadProfiles()
                        ...
                        self.connectionLogger.error("🔗 ❌ Blind HTTP fallback also failed: \(error)")
                    }
                }
            }
            ...
            if profile.isTailscaleEnabled {
                ...
                if let healthProfile = await checkServerHealth(for: profile) {
                    ...
                    self.connectionLogger.warning("🔗 ⚠️ Could not determine server state, updating profile to HTTP-only")
                    ...
                    ServerProfile.save(fallbackProfile, to: self.userDefaults)
                    await MainActor.run {
                        self.loadProfiles()
                    }
                }
