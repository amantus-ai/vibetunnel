...
function patchNodePty() {
  ...
  execSync('NODE_NO_WARNINGS=1 pnpm install node-pty --silent', { 
    stdio: 'inherit',
    env: {
      ...process.env,
      NODE_NO_WARNINGS: '1',
  ...
  if (customNodePath) {
    ...
    try {
      ...
      execSync(`pnpm rebuild node-pty`, {
        stdio: 'inherit',
        env: {
          ...process.env,
...
`;
      ...
      try {
        ...
        execSync(`pnpm install node-pty authenticate-pam --force`, { stdio: 'inherit' });
        
        // Then rebuild them with custom Node settings
        // Create a wrapper script to filter warnings
        const rebuildWrapperScript = `#!/bin/bash
# Filter out specific warnings while preserving errors
pnpm rebuild node-pty authenticate-pam "$@" 2>&1 | grep -v "cast from 'typename" | grep -v "converts to incompatible function type" | grep -v "expanded from macro" | grep -v "~~~" | grep -v "In file included from" | grep -v "warnings generated" | grep -v "In instantiation of" | grep -v "requested here" || true
`;
  ...
  const prebuildLoaderFile = path.join(__dirname, 'node_modules/node-pty/lib/prebuild-loader.js');
...
//# sourceMappingURL=prebuild-loader.js.map`;
  ...
  const windowsPtyAgentFile = path.join(__dirname, 'node_modules/node-pty/lib/windowsPtyAgent.js');
  ...
  const indexFile = path.join(__dirname, 'node_modules/node-pty/lib/index.js');
  ...
  const unixTerminalFile = path.join(__dirname, 'node_modules/node-pty/lib/unixTerminal.js');
...
function cleanPatches() {
  ...
  const filesToClean = [
    'node_modules/node-pty/lib/prebuild-loader.js',
  ...
  try {
    execSync('pnpm install node-pty --force', {
      cwd: __dirname,
      stdio: 'inherit'
    });
...
async function main() {
  try {
    ...
    execSync('rm -rf node_modules/node-pty', { stdio: 'inherit' });
    execSync('pnpm install node-pty --silent', { stdio: 'inherit' });
