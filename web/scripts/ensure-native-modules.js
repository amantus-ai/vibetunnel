...
const nativeForwarderPath = path.join(webRoot, 'native', 'shellops-fwd');
const binForwarderPath = path.join(webRoot, 'bin', 'shellops-fwd');
...
if (!fs.existsSync(nativeForwarderPath) || !fs.existsSync(binForwarderPath)) {
  ...
  try {
    ...
    console.error('zig is required to build shellops-fwd but was not found in PATH');
