...
describe('CLI Verbosity Environment Variables', () => {
  ...
  describe('SHELLOPS_LOG_LEVEL', () => {
    ...
    it('should parse error level', async () => {
      process.env.SHELLOPS_LOG_LEVEL = 'error';
      const level = await getVerbosityFromEnv();
    ...
    it('should parse verbose level', async () => {
      process.env.SHELLOPS_LOG_LEVEL = 'verbose';
      const level = await getVerbosityFromEnv();
  ...
  describe('SHELLOPS_DEBUG (backward compatibility)', () => {
    ...
    it('should NOT override valid SHELLOPS_LOG_LEVEL when set', async () => {
      process.env.SHELLOPS_LOG_LEVEL = 'warn';
      process.env.SHELLOPS_DEBUG = '1';
      const level = await getVerbosityFromEnv();
      expect(level).toBe(VerbosityLevel.WARN); // SHELLOPS_LOG_LEVEL takes precedence
    });
  });
});
